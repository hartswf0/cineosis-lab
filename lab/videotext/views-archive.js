...
R('079', 'Filter Columns', 'Tick boxes in three columns (decade, source, grade); the shots on the right keep up.', (box, v, o) => {
  ...
  const col = k => `<div style="min-width:0"><div class="vlab">${k}</div>${F[k].map(([x, n]) => `<label style="display:flex;gap:6px;font-size:11.5px;align-items:center;cursor:pointer;padding:1px 0;min-width:0"><input type="checkbox" data-c="${k}" value="${esc(x)}" ${st[k].has(x) ? 'checked' : ''}> <span style="min-width:0;overflow-wrap:anywhere">${esc(x)}</span> <small style="color:var(--dim)">${n.length}</small></label>`).join('')}</div>`;
