...
- **Syzygy · the spine's vertebrae** (`lab/syzygy/`, open `lab/syzygy/index.html`; built by `build_vertebrae.py` then `build_view.py`) — every one of the 605 spine elements placed on her clock (422 by her own words; the rest by their timestamps on another master, interpolated between matched neighbours, ≈0.5 s leave-one-out error), plus 77 storyboard boards between lines. Each vertebra carries her words and line recording, the beat and its asked signs, the prompt and syntagma it was generated under, every image variant with its sign reading and nearest archive shots, the four `plan.py` cuts' shots with Metz's reading, the Spine-Cut picks, the beat's pool, the forage cut's shot and the genome patch, and where those layers agree. The page shows the alignment itself: every body (her voice, the generated images, the storyboard, the four `plan.py` cuts, Spine-Cut, the forage cut) is a lane of its clips sliding under one vertical line at the moment of her clock. A body whose sign the beat asks for lights on the line in that sign's colour, and when four or more line up the line glows. Drag to move, pinch to zoom, tap a clip to bring it to the line and again to open it; ▶ plays through with her recording where it loads. The strip above is the alignment across all 24 minutes (`build_lanes.py` → `lanes.json`).
